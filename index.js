...
(function () {
   ...
   */
     ...
     */
        ...
        // Converts the queue to an array, handling both node objects and direct data storage
        toArray: function () {
            var array = this._list.toArray();
            if (array.length > 0 && typeof array[0].getData === 'function') {
                // If the array contains nodes with getData method
                return array.map(function (node) {
                    return node.getData();
                });
            } else {
                // If the array already contains the data
                return array;
            }
        },
        ...
        reverse: function () {
            ...
            for (i = 0; i < reversedArray.length; i++) {
                this._list.insert(reversedArray[i]);
            }
        },
    };
